...
public class ScrollLeftAdapter extends BaseQuickAdapter<String, BaseViewHolder> {
    ...
    protected void convert(BaseViewHolder helper, String item) {
        helper.setText(R.id.list_shop_left_text, item)
                .addOnClickListener(R.id.item);
        //将左侧item中的TextView添加到集合中
        tv.add((TextView) helper.getView(R.id.list_shop_left_text));
        //设置进入页面之后,左边列表的初始状态
...
//                tv.get(i).setBackgroundColor(0xff0068b7);
                tv.get(i).setTextColor(ContextCompat.getColor(mContext, R.color.text_efb134));
...
//                tv.get(i).setBackgroundColor(0xffffffff);
                tv.get(i).setTextColor(ContextCompat.getColor(mContext, R.color.tab_normal_color));
